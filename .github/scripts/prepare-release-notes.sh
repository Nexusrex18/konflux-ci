...
# get_previous_tag finds the most recent stable release tag before the current
# release. Uses --sort=-version:refname for semantic version ordering (works
# across release branches) and a strict regex to match only stable semver tags
# (vX.Y.Z) — excluding pre-release suffixes like -rc, -beta, -alpha, etc.
# Excludes $VERSION so re-runs after tag creation still return the previous tag.
get_previous_tag() {
